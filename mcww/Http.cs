...
http.cs is free software; you can redistribute it and/or
modify it under the terms of the GNU General Public License as
published by the Free Software Foundation; either version 2 of the
License, or (at your option) any later version.

http.cs was developed by Carlos Justiniano for use on the
...
along with http.cs; if not, write to the Free Software 
...
namespace mcww
{
	public class HTTPMessage
	{
		...
		{
			get { return messageHttpVerb; }
			set { messageHttpVerb = value; }
		}
		...
		private string messageHttpVerb;
		...
		public void Send(int timeOut)
		{
			...
			if (commandLine.IndexOf("POST ") == -1 && commandLine.IndexOf("GET ") == -1)
			{
				msgHeader = (messageHttpVerb.IndexOf("POST") == 0) ? "POST " : "GET ";
				//msgHeader += url.Path;
			}
			...
			msgHeader += " HTTP/1.1\r\n";
			msgHeader += "Host: http://";
			msgHeader += url.Host + ":" + url.Port.ToString();
			msgHeader += "\r\n";
		...
		public void SendRecieve(int timeOut)
		{
						...
						for (int i= 0; i < tmp.Length - 3; i++)
						{
							if (tmp[i] == '\r' && tmp[i+1] == '\n' && tmp[i+2] == '\r' && tmp[i+3] == '\n')
							{
								...
								if (idx != -1)
								{
									...
									if (val.IndexOf("\r") != -1)
										val = val.Substring(0, val.IndexOf("\r"));
